...
int main () {
    ...
    for (int i = 0; i < D; i++) {
        ll m = ds[i].m;
        ll b = ds[i].b;
        ll sum = st2.query(1, 1, maxN, 1, m);
        // cout << "sum " << sum << " b: " << b << endl;
        if (b <= sum) {
            cout << st.query(1, 1, maxN, 1, maxN)%MOD2 << endl;
            continue;
        }
        b-=sum;
        // cout << "m: " << m << " b: " << b << endl;
        while (b>0) {
            if (ss.empty()) {
                b+=st2.query(1, 1, maxN, 1, 1)*(m);
                if (b%m!=0) {
                    ss.insert({b%m, b/m+1});
                    st.setit(1, 1, maxN, 1, b%m, binpow(3, b/m+1));
                    st2.setit(1, 1, maxN, 1, b%m, b/m+1);
                }
                ss.insert({m, b/m});
                st.setit(1, 1, maxN, b%m+1, m, binpow(3, b/m));
                st2.setit(1, 1, maxN, b%m+1, m, b/m);
            ...
            ll x = s.x;
            ll v = s.v;
            b+=st2.query(1, 1, maxN, x+1, x+1)*(m-x);
            ...
            if (b <= (m-x)*v) {
                ...
                if (hm == v) {
                    ss.erase(s);
                    // cout << "erased: " << x << " " << v << endl;
                }
                if (d_!=0) {
                    ss.insert({x+d_, hm});
                    st.setit(1, 1, maxN, x+1, x+d_, binpow(3, hm));
                    st2.setit(1, 1, maxN, x+1, x+d_, hm);
                    // cout << "setit: " << x+1 << " " << x+d_ << " " << hm << endl;
                }
                ss.insert({m, b/(m-x)});
                st.setit(1, 1, maxN, x+d_+1, m, binpow(3, b/(m-x)));
                st2.setit(1, 1, maxN, x+d_+1, m, b/(m-x));
                // cout << "setit: " << x+d_+1 << " " << m << " " << b/(m-x) << endl;
                b = 0;
            } else {
                b-=(m-x)*(v);
                if (x<m) {
                    st.setit(1, 1, maxN, x+1, m, binpow(3, v));
                    st2.setit(1, 1, maxN, x+1, m, v);
        ...
        ll ans = st.query(1, 1, maxN, 1, maxN)%MOD2;
        cout << ans << endl;
    }
}