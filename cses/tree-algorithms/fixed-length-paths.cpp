...
void build(int u, int p, vvi& adj, vt<bool>& rem, vi& szs, vi& par) {
    ...
    if (m != -1) {
        ...
        rep(i,0,sz(deps)) {
            rep(j,1,sz(deps[i])) {
                // cerr << "x: " << deps[i][j] << ", y: " << ((K-j<m?tot[K-j]:0) - (K-j<sz(deps[i])?deps[i][K-j]:0)) << endl;
                ans += (ll)deps[i][j] * ((K-j<m&&K-j>=0?tot[K-j]:0) - (K-j<sz(deps[i])&&K-j>0?deps[i][K-j]:0));
            }
        }
        rep(i,0,sz(deps)) {
            if (K < sz(deps[i])) {
